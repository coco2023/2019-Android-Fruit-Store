        ...
        public void onClick(View v) {
            try {
                if (!mp.isPlaying()) { //没有播放，此时click应该执行播放
                    mp.start();
                    Toast.makeText(MediaActivity.this, "播放音乐", Toast.LENGTH_SHORT).show();
//                  更改按钮图标为暂停
                    Start.setImageResource(R.drawable.music_pause);
                } else {
                    mp.pause();
                    Toast.makeText(MediaActivity.this, "暂停播放音乐", Toast.LENGTH_SHORT).show();
//                  更改按钮图标为暂停
...
//            停止按钮事件，停止播放音乐，不是暂停
            ...
            //            便于用户再次选择播放
            try {
                mp = MediaPlayer.create(MediaActivity.this, R.raw.abc);
                mp.setLooping(true);
                Toast.makeText(MediaActivity.this, "停止播放音乐", Toast.LENGTH_LONG).show();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
